...
class NN:
    def __init__(self, dimension, neuron_num,
                 activation_func=None, network_type="QNN"):
        """
        ...
        :param activation_func: dictionary, { layer index : function }
            ...
            and output layers.
        :param network_type: which type of neural network: "QNN" or "CNN"
        """
    ...
    def set_activation_func(self, activation_func):
        """
        Help function to set the activation function of each layer.

        If "activation_func" is not specified when initial the NN object,
        the ReLu function is used as the activation function, and if it's last
        layer, the softmax function is used instead.
        """
        if activation_func is not None: return activation_func

        activation_func = {}
        for l in range(self.L - 1):
            activation_func[l] = self.relu
        activation_func[self.L-1] = self.softmax
    ...
    """ Optimizer: How to update the gradient """
    ...
    def train(self, train_point, train_label,
        ...
        for _ in range(epoch):
            ...
            for _ in range(step_size):
                grad = self.gradient(train_point, train_label)
                self.optimizer(optimizer, grad)
